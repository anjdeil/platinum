            ...
            {isLoading ? (
                ...
                <>
                    ...
                    <OrderSummaryLine>
                        ...
                        <span>{formatPrice(subtotal)}&nbsp;{symbol}</span>
                    </OrderSummaryLine>
                    ...
                    {order?.shipping_lines?.map((line) => (
                        <OrderSummaryLine key={line.id}>
                            ...
                            <span>{line.total}&nbsp;{symbol}</span>
                        </OrderSummaryLine>
                    ))}

                    {/* сoupons */}
                    {order?.coupon_lines?.map((line) => {
                        const name = `${t('сoupon')} ${line.discount_type === 'percent' ? `-${line.nominal_amount}% ` : ''
                            }`;
                        return (
                            <OrderSummaryLine key={line.id}>
                                <OrderSummaryLineName>
                                ...
                                <span>- {line.discount}&nbsp;{symbol}</span>
                            </OrderSummaryLine>
                        );
                    })}
                    ...
                    {order?.tax_lines?.map((line) => (
                        <OrderSummaryLine key={line.id}>
                            ...
                            <span>{line.tax_total}&nbsp;{symbol}</span>
                        </OrderSummaryLine>
                    ))}
                    ...
                    <OrderSummaryTotal>
                        ...
                        <OrderSummaryTotalValue>{order?.total || '—'}&nbsp;{symbol}</OrderSummaryTotalValue>
                    </OrderSummaryTotal>
                </>
            )}
        </OrderSummaryWrapper>
